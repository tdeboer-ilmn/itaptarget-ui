...
import { itapStudyUrl, itapVariantUrl } from '../../../utils/urls';
...
import ITAP_QUERY from './sectionQuery.gql';
...
const columns = [
  ...
  },
  {
    ...
    renderCell: ({ diseaseFromSource, studyId }) => {
      ...
      return (
        <Link external to={itapStudyUrl(studyId)}>
          {diseaseFromSource ? studyId : parsedDiseaseFromSource}
        </Link>
      );
    },
  },
  ...
  },
  {
    ...
    renderCell: ({ variantId, variantRsId }) => (
      <>
        {variantId ? (
          <Link external to={itapVariantUrl(variantId)}>
            {variantId}
          </Link>
        ) : (
          naLabel
...
function Body({ definition, id: { ensgId, efoId }, label: { symbol, name } }) {
  ...
  } = usePlatformApi(Summary.fragments.iTAPSummaryFragment);
  const variables = { ensemblId: ensgId, efoId, size };

  const request = useQuery(ITAP_QUERY, {
    variables,
  });
    ...
    <SectionItem
      ...
      renderBody={data => (
        <DataTable
          columns={columns}
          dataDownloader
          dataDownloaderFileStem={`itap-${ensgId}-${efoId}`}
          order="desc"
          ...
          query={ITAP_QUERY.loc.source.body}
          variables={variables}
        />
      )}
    />
